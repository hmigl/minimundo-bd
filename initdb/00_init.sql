...
CREATE INDEX IDX_TB_ATIVIDADE_ID_SALA ON TB_ATIVIDADE (ID_SALA);
CREATE INDEX IDX_TB_ATIVIDADE_DT_INICIO ON TB_ATIVIDADE (DT_INICIO);
--
CREATE INDEX IDX_TB_INSCRICAO_ID_PARTICIPANTE ON TB_INSCRICAO (ID_PARTICIPANTE);
CREATE INDEX IDX_TB_INSCRICAO_ID_ATIVIDADE ON TB_INSCRICAO (ID_ATIVIDADE);
--
CREATE INDEX IDX_TB_PRESENCA_ST_PRESENCA ON TB_PRESENCA (ST_PRESENCA);
--
CREATE INDEX IDX_TB_USO_MATERIAL_ID_ATIVIDADE ON TB_USO_MATERIAL (ID_ATIVIDADE);
CREATE INDEX IDX_TB_USO_MATERIAL_ID_MATERIAL ON TB_USO_MATERIAL (ID_MATERIAL);
--
CREATE INDEX IDX_TB_TAREFA_VOLUNTARIO_ID_VOLUNTARIO ON TB_TAREFA_VOLUNTARIO (ID_VOLUNTARIO);
CREATE INDEX IDX_TB_TAREFA_VOLUNTARIO_ID_ATIVIDADE ON TB_TAREFA_VOLUNTARIO (ID_ATIVIDADE);
--
CREATE INDEX IDX_TH_HISTORICO_PARTICIPACAO_ID_PARTICIPANTE ON TH_HISTORICO_PARTICIPACAO (ID_PARTICIPANTE);
CREATE INDEX IDX_TH_HISTORICO_PARTICIPACAO_ID_ATIVIDADE ON TH_HISTORICO_PARTICIPACAO (ID_ATIVIDADE);
CREATE INDEX IDX_TH_HISTORICO_PARTICIPACAO_DT_CONCLUSAO ON TH_HISTORICO_PARTICIPACAO (DT_CONCLUSAO);
--
CREATE INDEX IDX_TB_AVALIACAO_INSTRUTOR_ID_INSTRUTOR ON TB_AVALIACAO_INSTRUTOR (ID_INSTRUTOR);
CREATE INDEX IDX_TB_AVALIACAO_INSTRUTOR_ID_ATIVIDADE ON TB_AVALIACAO_INSTRUTOR (ID_ATIVIDADE);
--
CREATE INDEX IDX_TB_PATROCINIO_ID_PARCEIRO ON TB_PATROCINIO (ID_PARCEIRO);
CREATE INDEX IDX_TB_PATROCINIO_ID_EVENTO ON TB_PATROCINIO (ID_EVENTO);
--
CREATE INDEX IDX_TB_EVENTO_DT_INICIO ON TB_EVENTO (DT_INICIO);
--
CREATE INDEX IDX_TB_MATERIAL_QTD_TOTAL ON TB_MATERIAL (QTD_TOTAL);
...
CREATE TABLE TA_AUDITORIA (
  ...
  NM_TABELA_ORIGEM        VARCHAR(30)      NOT NULL,
  PK_REGISTRO_ORIGEM      VARCHAR(100)     NOT NULL,
  TP_OPERACAO             CHAR(1)          NOT NULL CHECK (TP_OPERACAO IN ('I','U','D')),
  DT_OPERACAO             TIMESTAMP        NOT NULL DEFAULT CURRENT_TIMESTAMP,
  NM_USUARIO_BD           VARCHAR(50)      NOT NULL DEFAULT SESSION_USER,
  NM_USUARIO_APLICACAO    VARCHAR(50)      NOT NULL,
  NM_TERMINAL             VARCHAR(50)      NOT NULL,
  DS_DADOS_ANTERIORES     TEXT,
  DS_DADOS_NOVOS          TEXT
);

-- Função genérica de auditoria
CREATE OR REPLACE FUNCTION FC_INS_AUDITORIA()
  RETURNS TRIGGER
  LANGUAGE plpgsql
AS $$
DECLARE
  ...
  v_oldj JSON;
BEGIN
  -- Converte ROW para JSON uma única vez
  v_newj := to_json(NEW);
  v_oldj := to_json(OLD);

  -- Extrai a PK conforme o nome da tabela
  v_pk := CASE TG_TABLE_NAME
    WHEN 'tb_participante'      THEN COALESCE(v_newj->>'id_participante', v_oldj->>'id_participante')
    WHEN 'tb_sala'              THEN COALESCE(v_newj->>'id_sala',        v_oldj->>'id_sala')
    WHEN 'tb_evento'            THEN COALESCE(v_newj->>'id_evento',      v_oldj->>'id_evento')
    WHEN 'tb_instrutor'         THEN COALESCE(v_newj->>'id_instrutor',   v_oldj->>'id_instrutor')
    WHEN 'tb_material'          THEN COALESCE(v_newj->>'id_material',    v_oldj->>'id_material')
    WHEN 'tb_atividade'         THEN COALESCE(v_newj->>'id_atividade',   v_oldj->>'id_atividade')
    WHEN 'rl_atividade_instrutor' THEN COALESCE(v_newj->>'id_atividade', v_oldj->>'id_atividade') || ',' || COALESCE(v_newj->>'id_instrutor', v_oldj->>'id_instrutor')
    WHEN 'tb_inscricao'         THEN COALESCE(v_newj->>'id_inscricao',   v_oldj->>'id_inscricao')
    WHEN 'tb_presenca' THEN COALESCE(v_newj->>'id_inscricao', v_oldj->>'id_inscricao') || ',' || COALESCE(v_newj->>'dt_sessao',     v_oldj->>'dt_sessao')
    WHEN 'tb_certificado'       THEN COALESCE(v_newj->>'id_certificado', v_oldj->>'id_certificado')
    WHEN 'tb_feedback'          THEN COALESCE(v_newj->>'id_feedback',    v_oldj->>'id_feedback')
    WHEN 'tb_parceiro'          THEN COALESCE(v_newj->>'id_parceiro',    v_oldj->>'id_parceiro')
    WHEN 'tb_patrocinio'        THEN COALESCE(v_newj->>'id_patrocinio',  v_oldj->>'id_patrocinio')
    WHEN 'th_historico_participacao' THEN COALESCE(v_newj->>'id_historico_participacao', v_oldj->>'id_historico_participacao')
    WHEN 'tb_voluntario'        THEN COALESCE(v_newj->>'id_voluntario',   v_oldj->>'id_voluntario')
    WHEN 'tb_tarefa_voluntario' THEN COALESCE(v_newj->>'id_tarefa_voluntario', v_oldj->>'id_tarefa_voluntario')
    WHEN 'tb_uso_material'      THEN COALESCE(v_newj->>'id_uso_material',  v_oldj->>'id_uso_material')
    WHEN 'tb_avaliacao_instrutor' THEN COALESCE(v_newj->>'id_avaliacao_instrutor', v_oldj->>'id_avaliacao_instrutor')
    ELSE 'unknown'
  END;

  INSERT INTO ta_auditoria ( nm_tabela_origem, pk_registro_origem, tp_operacao, dt_operacao, nm_usuario_aplicacao, nm_terminal, ds_dados_anteriores, ds_dados_novos
  ) VALUES (
    TG_TABLE_NAME,
    v_pk,
    CASE TG_OP
    ...
    CASE WHEN TG_OP IN ('UPDATE','DELETE') THEN v_oldj::TEXT ELSE NULL END,
    CASE WHEN TG_OP IN ('INSERT','UPDATE') THEN v_newj::TEXT ELSE NULL END
  );

  -- Retorna o registro para que o INSERT/UPDATE/DELETE prossiga
...
-- Triggers de auditoria para TB_PARTICIPANTE
CREATE TRIGGER TG_A_I_TB_PARTICIPANTE
  AFTER INSERT ON TB_PARTICIPANTE
  FOR EACH ROW EXECUTE FUNCTION FC_INS_AUDITORIA();
...
-- === Extensão e configurações de auditoria (PPP1) ===
